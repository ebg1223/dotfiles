---
description: >-
  Default fast junior implementer for well-specified, low-risk tasks — mechanical
  edits, isolated bug fixes, renames, test additions, and small wiring changes after
  the approach is decided. Use when the plan is clear and scope is bounded to a few
  files. Prefer this over codex-junior-engineer or glm-junior-engineer unless the user
  explicitly requests those agents.
mode: subagent
model: cliproxyapi/grok-composer-2.5-fast
